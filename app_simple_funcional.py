...
# Configuración de la página - la más simple posible
st.set_page_config(
...
# Solo ocultar elementos básicos de Streamlit
...
# Sidebar simple y funcional - SIN CSS COMPLEJO
st.sidebar.title("🔍 Filtros y Acciones")
...
st.sidebar.write("Link permanente:")
st.sidebar.code("paolaies.streamlit.app")
...
if "show_new_student" in st.session_state and st.session_state.show_new_student:
    ...
    with col2:
        ...
        asistencia_dias = st.number_input("Días de clase en el trimestre:", min_value=1, max_value=100, value=20)
        dias_presentes = st.number_input("Días presentes:", min_value=0, max_value=asistencia_dias, value=asistencia_dias)
    ...
    with col1:
        ...
        for i in range(1, 4):
            with st.expander(f"Evaluación {i}"):
                ...
                calificacion = st.selectbox(f"Calificación {i}:", list(dm.calificaciones.keys()), key=f"cal_{i}")
                if nombre_eval:
